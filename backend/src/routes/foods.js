...
export default async function foodsRoutes(fastify) {
  ...
  fastify.get('/api/foods', async (request) => {
    ...
    if (q) {
      const { rows } = await pool.query(
        `SELECT id, name, kcal_per_100g, barcode FROM foods
    ...
    const { rows } = await pool.query(
      `SELECT id, name, kcal_per_100g, barcode FROM foods WHERE user_id IS NULL OR user_id = $1 ORDER BY name ASC`,
      [userId]
    );
    return rows;
  });
  ...
  fastify.post('/api/foods', async (request, reply) => {
    const { name, kcal_per_100g, barcode } = request.body || {};
    ...
    const { rows } = await pool.query(
      'INSERT INTO foods (id, user_id, name, kcal_per_100g, barcode) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, kcal_per_100g, barcode',
      [id, request.user.sub, name, kcal_per_100g, barcode || null]
    );
    return rows[0];
  });
  ...
  fastify.patch('/api/foods/:id', async (request, reply) => {
    const { name, kcal_per_100g, barcode } = request.body || {};
    const { rows } = await pool.query(
      `UPDATE foods SET name = COALESCE($1, name), kcal_per_100g = COALESCE($2, kcal_per_100g), barcode = $3
       WHERE id = $4 AND user_id = $5 RETURNING id, name, kcal_per_100g, barcode`,
      [name ?? null, kcal_per_100g ?? null, barcode ?? null, request.params.id, request.user.sub]
    );
